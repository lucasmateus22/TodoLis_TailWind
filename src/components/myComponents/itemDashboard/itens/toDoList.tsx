...
export default function ToList() {
    ...
    const handleAddTask = () => {
        if (!taskText.trim() || !taskTime.trim()) {
            setAlert({ title: "Erro de entrada", description: "O texto e a hora da tarefa não podem estar vazios." });
            setTimeout(() => {
                setAlert(null);
            }, 3000);
            return;
        ...
        clearIncomingData();
        setAlert({ title: "Sucesso!", description: "Tarefa adicionada com sucesso." });
        setTimeout(() => {
            setAlert(null);
        }, 3000);
    };
