import {
  Card,
  CardHeader,
  CardTitle,
  CardDescription,
  CardContent,
} from "@/components/ui/card";
import { useAuth } from "@/features/services/authProvider/authProvider";
import { useState } from "react";
import { auth } from "@/features/services/authProvider/firebase";
import { signInWithPopup, GoogleAuthProvider } from "firebase/auth";
import { Loader2 } from "lucide-react";

export default function LoginBox() {
  const [loadingBtn, setLoadingBtn] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const { login } = useAuth(); // vindo do AuthProvider

  const handleGoogleLogin = async () => {
    setLoadingBtn(true);
    setError(null);

    try {
      const provider = new GoogleAuthProvider();
      const result = await signInWithPopup(auth, provider);
      const user = result.user;
      await login(user);

    } catch (err: any) {
      setError("Erro ao fazer login com Google");
    } finally {
      setLoadingBtn(false);
    }
  };

  return (
    <Card className="w-96 min-h-[300px] shadow-lg border border-gray-200 bg-white">
      <CardContent>
        <form className="flex flex-col gap-4 mt-4">
          <CardHeader className="text-center mb-2">
            <CardTitle className="text-2xl font-bold text-gray-800">
              Welcome!
            </CardTitle>
            <CardDescription className="text-gray-500">
              Enter with Your Account for Continue
            </CardDescription>
          </CardHeader>

          {error && (
            <p className="text-red-500 text-sm text-center">{error}</p>
          )}

          <button
            type="button"
            onClick={handleGoogleLogin}
            disabled={loadingBtn}
            className="flex items-center justify-center gap-3 px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 font-medium hover:bg-gray-50 hover:shadow-md transition disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loadingBtn ? (
              <Loader2 className="animate-spin w-5 h-5" />
            ) : (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 48 48"
                className="w-5 h-5"
              >
                <path
                  fill="#EA4335"
                  d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 
                    30.47 0 24 0 14.62 0 6.51 5.38 2.56 
                    13.22l7.98 6.19C12.43 13.72 
                    17.74 9.5 24 9.5z"
                />
                <path
                  fill="#4285F4"
                  d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 
                    2.96-2.26 5.48-4.78 7.18l7.73 
                    6c4.51-4.18 7.09-10.36 
                    7.09-17.65z"
                />
                <path
                  fill="#FBBC05"
                  d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 
                    16.46 0 20.12 0 24c0 3.88.92 
                    7.54 2.56 10.78l7.97-6.19z"
                />
                <path
                  fill="#34A853"
                  d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 
                    1.45-4.92 2.3-8.16 
                    2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 
                    6.19C6.51 42.62 14.62 48 
                    24 48z"
                />
              </svg>
            )}
            {loadingBtn ? "Entrando..." : "Continuar com Google"}
          </button>
        </form>
      </CardContent>
    </Card>
  );
}
